...
myConfig p = def
        ...
        , handleEventHook    = refocusLastWhen myPred <+> myHandleEventHook
...
myStatusBar         = "xmobar -x0 /home/dypan/.xmobarrc"
--myLauncher          = "dmenu_run"
--myLauncher          = "rofi -matching fuzzy -show run"
myLauncher          = "rofi -matching fuzzy -modi combi -show combi -combi-modi window,run,drun"
...
myFont      = "xft:Noto Sans CJK SC:size=8:antialias=true"
myBigFont   = "xft:Noto Sans CJK SC:size=8:antialias=true"
myWideFont  = "xft:Noto Sans CJK SC:size=8:antialias=true"
...
--    myTall = subLayout [] Simplest $ trackFloating (Tall 1 (1/20) (1/2))
    ...
    myFlex = named "MyFlex"
         ...
         $ addTopBar
         $ windowNavigation
         $ subLayout [0,1,2] (tabbed shrinkText myTabTheme)
         $ Tall 1 (3/100) (1/2) ||| tabbed shrinkText myTabTheme
-- myLayout =  gaps $ avoidStruts $ windowNavigation $ boringWindows $ subLayout [0,1,2] (tabbed shrinkText myTabConfig) $ Tall 1 (3/100) (1/2) ||| tabbed shrinkText myTabConfig ||| noBorders Full


    -- myFlex = named "myFlex"
    --           $ avoidStruts
    --           $ windowNavigation
    --           -- $ subLayout [0,1,2] (addTabs shrinkText myTabTheme)
    --           $ Tall 1 (3/100) (1/2)
...
myKeys conf = let
    ...
    --] ^++^
    subKeys "System"
    [ ("M-z"                    , addName "Restart XMonad"                  $ spawn "xmonad --restart")
    , ("M-C-z"                  , addName "Rebuild & restart XMonad"        $ spawn "xmonad --recompile && xmonad --restart")
    , ("M-S-z"                  , addName "Quit XMonad"                     $ confirmPrompt hotPromptTheme "Quit XMonad" $ io (exitWith ExitSuccess))
    , ("M-x"                    , addName "Lock screen"                     $ spawn "xset s activate")
    , ("M-<F4>"                    , addName "Print Screen"                    $ return ())
    , ("M-<F9>", addName "" $ spawn "amixer -D pulse sset Master 5%+")
    , ("M-<F8>", addName "" $ spawn "amixer -D pulse sset Master 5%-")
    , ("M-<F10>",        addName "" $ spawn "amixer -D pulse sset Master toggle")
